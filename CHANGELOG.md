...
- The 6.2.4 → 6.2.5 in-place update silently no-op'd for several users: the EXE downloaded the new zip, extracted it, claimed "Extracting update..." in the log, and relaunched into the same 6.2.4 build. Three bugs were stacking. The post-extract bat called `taskkill /F /PID <pid>` and immediately ran `rmdir /s /q _internal`, which fired before Windows finished releasing file locks on `_internal\python313.dll`, `Qt6*.dll`, and the EXE itself, so the directory removal partially failed. Robocopy then ran with `/E /IS /IT` (additive mirror, no purge) so the leftover stale files from 6.2.4 stayed alongside whichever 6.2.5 files robocopy managed to write, producing a Frankenstein install whose import order resolved to the old build. Robocopy's exit code path also dropped into a `pause` inside a `DETACHED_PROCESS` cmd window the user could never see, so a fatal robocopy exit (`>= 8`) was indistinguishable from success. Updater rewrite: poll `tasklist /FI "PID eq …"` until the EXE is gone (30s ceiling so a stuck shutdown can't deadlock the bat), use `robocopy /MIR /R:2 /W:1` so stale files actually purge, exclude `.git`, `lumacore`, `manifest_backup`, `downloaded_files`, `manifests` from the mirror so user state isn't wiped, and the bat's own files don't get clobbered mid-copy. Every step writes to `tmp_updater.log` next to the EXE so a failed update leaves a paper trail. The relaunch only fires on a robocopy exit code below 8; anything `>= 8` aborts the install in place and leaves the log behind for triage
- A startup probe in the GUI now reads `tmp_updater.log` 2 seconds after the window paints, surfaces any FAIL / WARN line as a `QMessageBox.warning`, and deletes the log so the warning only fires once. A clean update writes an INFO summary the user can ignore. Headless bat windows can no longer swallow update failures silently
...
- LumaCore source no longer ships the compiled-in `PatternDb.h` byte-pattern arrays. Every hook installer now resolves its target through the runtime pattern fetcher's `<sha>.toml` lookup, with the analyzer-emitted file under `<Steam>\lumacore\pattern\` as the single source of truth. Removes the long-tail risk that an outdated compiled signature lands a hook on the wrong function on a freshly published Steam build, and shrinks the surface to one pattern source instead of two
- `SteamUI.cpp` no longer carries the `DecodeTopManagerGetter` rel32 walk. The canonical pattern publisher publishes `GetTopManager` with the rva pointing straight at the 2-instruction getter (`mov rax, [rip+disp]; ret`), so the resolved address is the function pointer directly. Drops the dead `__try`-wrapped instruction decoder and the build-id-pinned `TopManagerCall` anchor sig along with it
- `KeyValues::ReadAsBinary` and `KeyValues::FindOrCreateKey` hook bodies are now silent passthroughs. Earlier builds emitted `LOG_KEYVALUECH_INFO` on every fire, which produced 30+ MB of disk traffic in under 30 seconds once Steam loaded its app list. The Install / Uninstall log lines stay so triage on attach failures still surfaces
- LicenseHooks keeps `OptedInMask` and `RequiresLegacyCDKey` as the only two detoured surfaces. The five extra DLC / cloud / subscription detours that were briefly added (BIsDlcEnabled, IsAppDlcInstalled, IsCloudEnabledForApp, GetSubscribedApps, BUpdateAppOwnershipTicket, BUpdateLicenses) caused random Steam crashes after a few minutes of clicking through games and flipped cloud-save on for every Lua-tracked app. The patterns for those six still ride in the per-build TOML, so future code that needs their addresses can resolve them; what's gone is the hand-rolled x64 fastcall trampolines

### LumaCore — runtime pattern fetcher

- Pattern repo schema follows the canonical pattern publisher (`OpenSteam001/steam-monitor` `pattern` branch) byte-for-byte. Section keys are FNV-1a 32-bit of the function name (offset basis `0x811c9dc5`, prime `0x01000193`), filenames are the lowercase-hex SHA-256 of the inspected DLL, fields are `name` / `rva` / `sig`. Lets users drop a canonical TOML straight into `<Steam>\lumacore\pattern\` and have it resolve, no manual rekeying
- The runtime pattern fetcher's HTTPS client gained a 1 MiB body cap and a 10-second total WinHTTP timeout per request, and a per-`(subdir, sha)` memo so a re-run for the same DLL pair never re-hits the network in the same session. A misconfigured mirror serving a huge blob, a slow CDN, or a transient DNS failure can no longer stall the install pass
...
- LumaCore now resolves byte-pattern addresses at runtime against the Steam DLLs on disk instead of shipping compiled-in pattern arrays. On launch, two detached worker threads hash `steamclient64.dll` and `steamui.dll` (lowercase hex SHA-256, 64 chars) and pull a matching `<sha>.toml` from the user's `KoriaPolis/Steam-Auto-PT` repo on the `pattern` branch. Fallback chain runs GitHub raw → jsDelivr CDN → local cache under `<Steam>\lumacore\pattern\`, with the GitHub primary leg short-circuiting to the cache step on HTTP 404 since both mirrors serve the same content. The Steam loader thread never waits on the network. Existing compiled `*Sigs[]` arrays stay in place as a final fallback so a build with no network and no cache still boots
- Optional `[pattern_fetch] mirror` key in `lumacore.toml` lets users point LumaCore at a private mirror or intranet server. Substitutes `{subdir}` and `{sha}` placeholders into the URL and runs the mirror as an additive first try; on any failure (HTTP 4xx/5xx, network error, parse error) the fetcher logs a debug line and falls through to the regular GitHub primary chain
- Cache writes go through `<sha>.toml.tmp` followed by an atomic rename, so a writer crash, power loss, or concurrent reads from multiple processes never expose a partially written cache file. Surviving `.tmp` files from a crashed writer get swept on the next successful fetch into the same cache directory
- ByteScan consults the runtime pattern map before iterating the compiled signature arrays. A hit verifies the bytes at `module_base + entry.rva` against the TOML's sig before returning, so a stale or wrong RVA can never land hooks at a bad address. Out-of-range RVAs and byte mismatches log a warning and fall through to the compiled arrays
- Per-DLL graceful degradation: if the fetcher cannot resolve a TOML for `steamclient64.dll`, only hooks bound to `steamclient64.dll` skip; hooks bound to `steamui.dll` keep installing, and vice versa. The process never aborts on a missing or malformed pattern file

### LumaCore — robustness hardening

- Lua uint64 strings now route through a strict-decimal helper (`TryParseUInt64Decimal`) before reaching `std::stoull`. Empty input, embedded whitespace, signs, and `0x` prefixes get rejected upfront; any parse exception (`std::invalid_argument`, `std::out_of_range`) is caught so a malformed `.lua` config rejects with a `LOG_LUA_WARN` and a `luaL_error` instead of unwinding into Steam's loader. Covers the four Lua binding call sites that took user-supplied uint64 strings (token registration, manifest gid override, stat steamid set, plus the filename-stem auto-register in `LuaQuery.cpp`)
- Pattern fetcher now primes the in-memory map from the on-disk cache synchronously before any hook installs. Earlier builds dispatched the fetcher as a detached thread and started installing hooks before the worker finished, so even when `<sha>.toml` was already cached every hook fell through to the compiled `*Sigs[]` arrays. The two-phase order (sync cache prime, detached network refresh) means the fetcher actually resolves prologue addresses on the very first hook installer for every session after the initial download
- TOML name keys now match the bare hook identifiers (`ReadAsBinary`, `FindOrCreateKey`) instead of the namespaced `KeyValues_*` form. ByteScan still falls back to the prefixed names when consulting older pattern files so previously uploaded TOMLs keep working until they get re-emitted
- Dropped the warning-level log line that fires when the post-login Lua injection beats the package-info capture by a millisecond. The retry path inside `GetPackageInfo`'s hook re-runs the injection the moment the pointer arrives, so the line is benign startup ordering, not a hook failure. Kept at debug instead
- DirWatch caps the configured-directory list at the Win32 `MAXIMUM_WAIT_OBJECTS` limit (typically 64) before entering the wait loop. Beyond the cap the watcher used to die silently with `WAIT_FAILED`; it now truncates the list with a logged warning containing the original count and the cap value. Empty configured-directory lists exit the watcher within 1 ms without calling any Win32 wait API, and `DirWatch::Start` returns immediately without dispatching the worker thread when no directories are configured
- DllMain pins the LumaCore module on attach via `GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_PIN | GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, ...)` so a stray `FreeLibrary` cannot unmap the DLL while hooks and worker threads are still active. On `DLL_PROCESS_DETACH` with `pvReserved != nullptr` (process termination), the detach path now calls only `DirWatch::Stop()` and skips MinHook teardown, since the loader lock is held during process termination and touching MinHook from there can deadlock. Graceful FreeLibrary path keeps the existing `DirWatch::Stop()` → `SteamUI::CoreUnhook()` → `LumaCore::Detach()` order
